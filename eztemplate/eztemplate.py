            ...
            match template[ptr]:
                case '#':
                    if template[max(0, ptr - 1)] != '\\' and template[ptr + 1:ptr + 6] != '{end}':
                        ...
                        for loc[f"index_{self.__loop_counter}"], element in enumerate(res):
                            ...
                            to_sub += self.__render(data_to_render, **kwargs).lstrip()
                ...
                case '?':
                    if template[max(0, ptr - 1)] != '\\' and template[ptr + 1:ptr + 6] != '{end}':
                        ...
                        to_sub = ""
                        if res: to_sub += self.__render(data_to_render, **kwargs).strip()
...
def render_template(filename, **kwargs):
    new_path = os.path.join(os.getcwd(), "templates", filename)
    if os.path.isfile(new_path) and "/" not in filename and ".." not in filename:
